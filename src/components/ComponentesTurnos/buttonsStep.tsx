...
const StepButtons: React.FC<StepButtonsProps> = ({ prevButtonText, nextButtonText }) => {
    ...
    return (
        <div className="appointBoxButtons">
            ...
            {currentStep < 5 && <button className='next' style={{ backgroundColor: `${hexToRgb(config.customization.primary.color)}`, color: `${config.customization.primary.text}` }} onClick={nextStep}>
                {nextButtonText}
